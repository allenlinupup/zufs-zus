...
static int zus_start_all_threads(struct thread_param *tp)
{
	int i, err;
	...
	g_zts = calloc(tp->num_cpus, sizeof(*g_zts));
	if (!g_zts)
		return ENOMEM;
	g_num_gts = tp->num_cpus;
	pthread_key_create(&g_zts_id_key, NULL);

	err = fba_alloc(&g_wait_structs,
		  tp->num_cpus * sizeof(struct zufs_ioc_wait_operation));
	if (unlikely(err)) {
		ERROR("fba_alloc => %d\n", err);
		return err;
	}

	wtz_arm(&g_wtz, tp->num_cpus);
