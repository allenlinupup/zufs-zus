...
/* ~~~ wrappers ~~~ to call the proper fs function pointer */
static
struct zus_sb_info *_zusfs_sbi_alloc(struct zus_fs_info *zfi)
{
	return zfi->op->sbi_alloc(zfi);
}

static
void _zusfs_sbi_free(struct zus_sb_info *sbi)
{
	sbi->zfi->op->sbi_free(sbi);
}

static
int _zusfs_sbi_init(struct zus_sb_info *sbi, struct zufs_ioc_mount *zim)
{
	return sbi->zfi->op->sbi_init(sbi, zim);
}

static int _zusfs_sbi_fini(struct zus_sb_info *sbi)
{
	// zus_iput(sbi->z_root); was this done already
	if (!sbi->zfi->op->sbi_fini)
		return 0;
	return sbi->zfi->op->sbi_fini(sbi);
}

static
struct zus_inode_info *_zusfs_zii_alloc(struct zus_sb_info *sbi)
{
	return sbi->op->zii_alloc(sbi);
}

static int _zusfs_new_inode(struct zus_sb_info *sbi, struct zus_inode_info *zii,
			    void* app_ptr, struct zufs_ioc_new_inode *ioc_new)
{
	return sbi->op->new_inode(sbi, zii, app_ptr, ioc_new);
}
static void _zusfs_zii_free(struct zus_inode_info *zii)
{
	zii->sbi->op->zii_free(zii);
}

static int _zusfs_free_inode(struct zus_inode_info *zii)
{
	return zii->sbi->op->free_inode(zii);
}

static ulong _zusfs_lookup(struct zus_inode_info *dir_ii, struct zufs_str *str)
{
	return dir_ii->sbi->op->lookup(dir_ii, str);
}
static int _zusfs_iget(struct zus_sb_info *sbi, struct zus_inode_info *zii,
		       ulong ino)
{
	return sbi->op->iget(sbi, zii, ino);
}
static int _zusfs_add_dentry(struct zus_inode_info *dir_ii,
			     struct zus_inode_info *zii, struct zufs_str *str)
{
	return dir_ii->sbi->op->add_dentry(dir_ii, zii, str);
}

static int _zusfs_remove_dentry(struct zus_inode_info *dir_ii,
				struct zufs_str *str)
{
	return dir_ii->sbi->op->remove_dentry(dir_ii, str);
}

static void _zusfs_evict_zii(struct zus_inode_info *zii)
{
	if (!zii->op->evict)
		return;
	zii->op->evict(zii);
...
static int _zusfs_read(ulong *app_ptr, struct zufs_ioc_hdr *hdr)
{
...
static int _zusfs_do_write(ulong *app_ptr, struct zufs_ioc_hdr *hdr)
{
	struct zufs_ioc_IO *io = (void *)hdr;
	struct zus_inode_info *zii = io->zus_ii;
...
static int zusfs_get_symlink(struct zus_inode_info *zii, void **symlink)
{
	return zii->op->get_symlink(zii, symlink);
}
...
int zus_mount(int fd, struct zufs_ioc_mount *zim)
{
	...
	sbi = _zusfs_sbi_alloc(zim->zus_zfi);
	if (unlikely(!sbi)) {
		err = -ENOMEM;
		goto err;
	}
...
struct zus_inode_info *zus_iget(struct zus_sb_info *sbi, ulong ino)
{
	...
	zii = _zusfs_zii_alloc(sbi);
	if (!zii)
		return NULL;

	zii->sbi = sbi;
	err = _zusfs_iget(sbi, zii, ino);
	if (err) {
		_zusfs_zii_free(zii);
		return NULL;
	}
...
static int _new_inode(void *app_ptr, struct zufs_ioc_hdr *hdr)
{
	...
	err = _zusfs_new_inode(sbi, zii, app_ptr, ioc_new);
	if (unlikely(err))
		goto _err_zii_free;
	...
	err = _zusfs_add_dentry(ioc_new->dir_ii, zii, &ioc_new->str);
	if (unlikely(err))
...
static int _evict(struct zufs_ioc_hdr *hdr)
{
	...
	if (unlikely(!ziei->zus_ii)) {
		ERROR("!ziei->zus_ii\n");
		return 0;
	}

	if (hdr->operation == ZUS_OP_FREE_INODE) {
		_zusfs_free_inode(ziei->zus_ii);
		 ...
		 *  fs->evict)
		 */
		if (!(ziei->flags & ZI_LOOKUP_RACE))
		/* not the new_ii error-case */
			_zusfs_evict_zii(ziei->zus_ii);
	}

	_zusfs_zii_free(ziei->zus_ii);
	return 0;
}
...
static int _lookup(struct zufs_ioc_hdr *hdr)
{
	...
	ino  = _zusfs_lookup(lookup->dir_ii, &lookup->str);
	if (!ino) {
...
static int _dentry(struct zufs_ioc_hdr *hdr)
{
	...
	if (hdr->operation == ZUS_OP_REMOVE_DENTRY)
		return _zusfs_remove_dentry(zid->zus_dir_ii, &zid->str);

	return _zusfs_add_dentry(zid->zus_dir_ii, zid->zus_ii, &zid->str);
}
...
int zus_do_command(void *app_ptr, struct zufs_ioc_hdr *hdr)
{
	...
	case ZUS_OP_READ:
		return _zusfs_read(app_ptr, hdr);
	case ZUS_OP_WRITE:
		return _zusfs_do_write(app_ptr, hdr);
